...
Furthermore, Lin et. al. state that "safety-net specialists did not achieve greater cumulative financial rewards due to MIPS's percentage-based adjustment structure, which disadvantages clinicians with smaller billing volumes" \cite{Lin2025Merit-BasedYears}. MIPS provides incentives for physicians to pursue certain kinds of treatments for their patients, but what if a patient needs treatment that MIPS doesn't reward? Patients may require a certain treatment that their doctor isn't financially incentivized by MIPS to provide. In essence, we aim to explore whether MIPS is financially incentivizing physician behavior that creates better short- and long-term outcomes for patients.
...
\subsection{Other}
Ultimately, our findings suggest a weak correlation between MIPS and patient outcomes. Our best K-means solution had a weak silhouette score, and the clusters did not suggest any clear groupings in the data. Similarly, our supervised findings also did not find a strong correlation either. Our findings suggest that because MIPS is not strongly correlated with patient outcomes, physicians are not financially motivated by the MIPS system to prescribe treatments that are most appropriate for their patients. In order to better serve patients (both directly and indirectly), MIPS should be adjusted to better align with positive patient outcomes in order to better serve Medicare enrollees as a whole.
...
\begin{itemize}
    \item ChatGPT Codex 0.147.0 (data loading and review)
