...
We discovered that 35.8\% of physician groups operated in more than one ZIP code, making it necessary to device a strategy to handle the overloap. MIPS results were connected to practice locations through the DAC directory, which was reduced to 227,313 unique organization ZIP pairs. Organization identifiers and ZIP codes were standardized before joining the datasets. 

Approximately 2.27\% of records lacked a usable ZIP after joining MIPS and DAC and could not be geographically assigned. The remaining ZIP codes were mapped to county FIPS codes. For ZIP codes spanning multiple counties, we used the crosswalk’s "tot\_ratio" to allocate a share of the ZIP level data to each county based on the percentage of addresses located there. The weighted contributions were normalized by the total allocation weight to produce comparable county level performance rates. After records without ZIP codes had been excluded, only 0.05\% of the remaining records lacked a county match. County performance was then aggregated separately for each MIPS measure and joined to county level Medicare outcomes for subsequent modeling.

This technique estimates the geographic distribution of group performance from practice address patterns, but it does not represent patient volume across counties.
...
For Figure \ref{fig:hba1c_readmit}, HbA1c was an inverse measure, so higher score represented worse performance. We subtracted 100 from the score so that higher scores represented better control, and evaluated hospital readmissions rate, noticing a weakly positive signal (better HbA1c led to decrease readmission rates).
