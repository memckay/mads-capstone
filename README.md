...
Requires Python 3.11 or newer (developed on 3.13 and 3.14).
...
Note that `data/processed/analytic_measure.csv.gz`, the county-by-measure table read by notebooks 03
and 04, is a pre-built input pinned in `data/manifest.yaml`; it is not regenerated by the pipeline, so
place it in `data/processed/` alongside the raw files. The pipeline runs in a few minutes once the
data is in place.
...
reports/      figures
scripts/      utility scripts (local helpers are gitignored)
requirements.txt
```
